...
export function Reports() {
  ...
  const fetchFilterOptions = async () => {
    try {
      const apiUrl = API_ENDPOINTS.FILTER_OPTIONS;
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const apiUrl = API_ENDPOINTS.KPI;
      const params = new URLSearchParams();
      ...
      // Add pagination parameters to get all data
      params.append('page', '1');
      params.append('limit', '1000000'); // Get all data like old system

      // For Product Batch Summary, we want all raw material records
      ...
      // Handle nested data structure - backend returns {data: [...]} format
  ...
  const fetchReportData = async (reportType: string, startDate: string, endDate: string) => {
    ...
    try {
      ...
      const apiUrl = API_ENDPOINTS.REPORTS;
      const params = new URLSearchParams();
      ...
      // Ensure we get all data without pagination limits for reports
      ...
      if (data && data.data && Array.isArray(data.data)) {
        ...
        // Data fetched successfully - no need to set view states anymore
      } else {
        ...
        <body>
          <div class="page-header">
            <div class="logo-container">
              <div class="logo-left">
                <img src="${herculesLogoBase64}" alt="Hercules Logo" />
              </div>
      ...
      <div className="space-y-6">
        ...
        <Card className="bg-slate-900/95 dark:bg-slate-900/95 bg-white/95 border-slate-700 dark:border-slate-700 border-slate-300">
          <CardHeader className="pb-2">
            <CardTitle className="text-cyan-300 dark:text-cyan-300 text-slate-700 flex items-center gap-2 text-base">
          ...
          <CardContent className="pt-0">
            <div className="grid grid-cols-1 md:grid-cols-6 gap-2 items-end">
              ...
              <Button
                ...
                disabled={loading}
                className="flex items-center gap-2 !bg-[#0088a9] hover:!bg-[#007b98] !text-white font-medium py-1 px-3 rounded-lg shadow-md transition-all duration-200 border border-transparent h-7 text-sm"
              >
            ...
            <CardContent className="pt-3 pb-3">
              <div className="flex flex-wrap gap-2 items-end">
                {activeTab === "Weekly" && (
                  <>
                    <div className="space-y-1">
                      <Label className="text-cyan-300 dark:text-cyan-300 text-slate-700 font-medium text-xs">Week Start Date:</Label>
                      <Input
                        type="datetime-local"
                    ...
                    <Button
                      className="bg-[#0088a9] hover:bg-[#007b98] !text-white font-medium py-1 px-3 rounded-[8px] shadow-md transition-all duration-200 h-7 text-sm"
                      onClick={applyFilters}
                      disabled={loading}
                    >
                ...
                {activeTab === "Monthly" && (
                  <>
                    <div className="space-y-1">
                      <Label className="text-cyan-300 dark:text-cyan-300 text-slate-700 font-medium text-xs">Month Start Date:</Label>
                      <Input
                        type="datetime-local"
                ...
                {activeTab === "Daily Report" && (
                  <>
                    <div className="space-y-1">
                      <Label className="text-cyan-300 dark:text-cyan-300 text-slate-700 font-medium text-xs">Select Date:</Label>
                      <Input
                        type="datetime-local"
        ...
        <div className="flex justify-end gap-3 py-2">
          <Button 
            onClick={handlePrint}
            disabled={loading}
            className="bg-[#0088a9] hover:bg-[#007b98] !text-white font-medium py-1.5 px-3 rounded-[8px] shadow-md transition-all duration-200 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Printer className="h-4 w-4 mr-2" />
            PRINT
          </Button>
          <Button
            className="bg-[#0088a9] hover:bg-[#007b98] !text-white font-medium py-1.5 px-3 rounded-[8px] shadow-md transition-all duration-200 text-sm"
            onClick={() => {
            ...
            {loading ? (
              ...
              <div className="overflow-x-auto w-full">
                <table className="w-full text-sm text-slate-900 dark:text-white border-collapse">
                  <thead className="bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-cyan-400 uppercase text-xs tracking-wider">
                    <tr>
                      {getTableHeaders(activeTab).map((header) => (
                        <th key={header} className="border border-slate-300 dark:border-slate-600 px-4 py-3 text-left w-auto font-semibold">{header}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
